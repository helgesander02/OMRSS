...
func Get_ShortestPath_Routing(network *network.OSRO_Network) *Paths_set {
	...
	for nth, flow := range network.FlowSet.TSNFlows {
		...
		path := ShortestPath(v2v_path, network.Graph_Set.TSNGraphs[nth], flow.Source, dest, network.BytesRate)
	...
	for nth, flow := range network.FlowSet.AVBFlows {
		dest := flow.Destinations[0] // Use first destination for point-to-point
		path := ShortestPath(v2v_path, network.Graph_Set.AVBGraphs[nth], flow.Source, dest, network.BytesRate)
	...
	for _, method := range network.FlowSet.EncapsulateMethod {
		for _, flow := range method.CAN2TTFlows {
			...
			if existingPath, ok := usedPath[key]; ok {
				...
				newPath.Method = method.Method_Name
				...
				path := ShortestPath(v2v_path, topo, flow.Source, flow.Destination, network.BytesRate)
				if path != nil {
					path.Method = method.Method_Name
				}
...
func findConnectionInTopology(node *topology.Node, toID int) *topology.Connection {
...
func (paths_set *Paths_set) InputPathSet(bgTSN int, bgAVB int) *Paths_set {
	input := new_Paths_Set()
...
func (paths_set *Paths_set) BGPathSet(bgTSN int, bgAVB int) *Paths_set {
	bg := new_Paths_Set()
